...
class AnimatedEntity : public Entity
{
	...
	std::map<std::string, std::vector<smk::Sprite>> animationGroups; //The list of all sprite groups. Each group represents a series of sprites that are part of an animation
...
public:
	...
	void AddSprite(std::string group,smk::Sprite sprite);

	//Constructs a new sprite and adds it to an animation group
	void EmplaceSprite(std::string group, smk::Texture& texture, Rect<int> rect, Vector2f scale = Vector2f(1.0f, 1.0f));
