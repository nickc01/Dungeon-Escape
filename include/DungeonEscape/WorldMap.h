...
class WorldMap : public Renderable
{
	...
	std::vector<Vector2f> enemySpawnPoints{}; //All the potential spots an enemy can spawn

	Vector2f doorLocation; //The location of the exit door

	Array2D<std::shared_ptr<BackgroundTile>> tiles; //The list of all tiles that make up the room

	Vector2<unsigned int> tileSize; //The size of each tile in the map

	Vector2<int> SpawnPoint; //The spawnpoint for the player
...
public:
	...
	Vector2<vectorType> GetDimensions()
	{
		return Vector2<vectorType>(static_cast<vectorType>(GetWidth()), static_cast<vectorType>(GetHeight()));
	}

	//Gets the spawnpoint of the player in the world map
	Vector2<int> GetSpawnPoint() const;
	//Gets all the enemy spawnpoints in the map
	const std::vector<Vector2f>& GetEnemySpawnPoints() const;
	//Gets all the enemy spawnpoints in the map
	std::vector<Vector2f>& GetEnemySpawnPoints();
	//Gets the location of the exit door
	Vector2f GetDoorLocation() const;
	...
	//Gets a tile at the specified coordinate. If no tile exists there, null will be returned
	BackgroundTile* operator[](Vector2<int> position) const;

	//Retrieves all the tiles that are wintin the specified rectangle
	Array2D<BackgroundTile*> GetTilesWithinRect(RectF rect) const;
