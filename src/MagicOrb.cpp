...
ResourceTexture MagicOrb::orbTexture; //The texture resource for the magic orb
...
MagicOrb::MagicOrb(const WorldMap& map, Vector2f spawnPoint, Vector2f direction) :
	...
	map(map)
{
	if (!orbTexture.Loaded())
	{
		orbTexture = ResourceTexture{ RES_POWERORB };
	}

	orbSprite = smk::Sprite(orbTexture.GetTexture());
	...
	auto texture = orbTexture.GetTexture();
	auto textureSize = Vector2i(texture.width(),texture.height());

	//Set the sprite's origin to be the center of the texture
	orbSprite.SetCenter(Vector2f(textureSize.x / 2u,textureSize.y / 2u));
...
void MagicOrb::Render(smk::Window& window)
{
	//If the orb is alive
	if (alive)
	{
