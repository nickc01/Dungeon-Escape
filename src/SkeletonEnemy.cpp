#include <DungeonEscape/SkeletonEnemy.h> //Contains the SkeletonEnemy class
...
namespace
{
	constexpr float REFRESH_TIME = 3.0f; //How fast the enemy skeleton should refresh its path to the player
...
ResourceTexture SkeletonEnemy::skeletonSheet; //The resource texture of the skeleton texture sheet

SkeletonEnemy::SkeletonEnemy(const WorldMap& map, Vector2f spawnPoint) :
	Enemy(map)
{
	if (!skeletonSheet.Loaded())
	{

		skeletonSheet = ResourceTexture{ RES_SKELETONSHEET };
	}
	...
	else
	{
		...
		for (auto orb : MagicOrb::GetAllOrbs())
		{
			...
			if (Math::EntitiesIntersect(*this, *orb))
			{
				...
				GetSprite()->SetColor(glm::vec4(128, 128, 255,255));
			}
		}
